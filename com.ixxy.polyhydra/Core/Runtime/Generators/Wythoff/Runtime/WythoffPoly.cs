...
namespace Polyhydra.Wythoff
{
    public class WythoffPoly
    {
        ...
        public double[]
            FaceCountAtVertexType; // number of faces at a vertex of each type (array of length FaceTypeCount)
        ...
        public WythoffPoly(int typeIndex, int p, int q)
        {
        ...
        public WythoffPoly(UniformTypes type)
        {
        ...
        public WythoffPoly(UniformTypes type, int p, int q)
        {
            string symbol = Uniform.Uniforms[((int)type) + 1].Wythoff;
            symbol = symbol.Replace("p", p.ToString()).Replace("q", q.ToString());
            UnpackSym(symbol);
            Calculate();
        }
...
//        }
        ...
        private void CalcFaces()
        {
            ...
            MinInRadius = 1 / Math.Abs(Math.Tan(M_PI / FaceSidesByType[IsHemi ? 1 : 0]) *
                                       Math.Tan(FundementalAngles[IsHemi ? 1 : 0]));
