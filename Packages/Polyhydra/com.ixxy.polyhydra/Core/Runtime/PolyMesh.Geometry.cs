...
namespace Polyhydra.Core
{
    public partial class PolyMesh
    {
        ...
        {
            var poly = FaceScale(new OpParams(1f)); // Splits faces
        ...
        public PolyMesh VertexOffset(OpParams o)
        {
            ...
            for (var vertexIndex = 0; vertexIndex < Vertices.Count; vertexIndex++)
            {
                if (!IncludeVertex(vertexIndex, o.filter)) continue;
                var offset = Vertices[vertexIndex].Normal * o.GetValueA(this, vertexIndex);
                poly.Vertices[vertexIndex].Position += offset;
            }
...
#pragma warning restore CS0168
        ...
        public PolyMesh AlphaShapes(OpParams o)
        {
            ...
            // Start with convex hull as base triangulation
            PolyMesh hull;
            try
            {
                hull = _ConvexHull(true);
            }
            catch
            {
                // If convex hull fails, return empty mesh
                return new PolyMesh();
            }
            ...
            for (int faceIndex = 0; faceIndex < hull.Faces.Count; faceIndex++)
            {
                ...
                // Calculate circumradius of this triangle
                float circumradius = CalculateTriangleCircumradius(
                    faceVerts[0].Position,
                    faceVerts[1].Position,
                    faceVerts[2].Position
                );

                // Keep triangle if its circumradius is <= alpha
                if (circumradius <= alpha)
                {
                    var faceVertIndices = new List<int>();
                    ...
                    foreach (var vert in faceVerts)
                    {
                        ...
                        if (!vertexIndexMap.ContainsKey(originalIdx))
                        {
                            vertexIndexMap[originalIdx] = vertexPoints.Count;
                            vertexPoints.Add(vert.Position);
                        }
                        faceVertIndices.Add(vertexIndexMap[originalIdx]);
                    }

                    if (faceVertIndices.Count == 3)
                    {
                        faceIndices.Add(faceVertIndices);
                    }
                }
            }

            if (vertexPoints.Count == 0 || faceIndices.Count == 0)
            {
                return new PolyMesh();
            }
        ...
        private float CalculateTriangleCircumradius(Vector3 a, Vector3 b, Vector3 c)
        {
            ...
            float areaSquared = s * (s - sideA) * (s - sideB) * (s - sideC);

            // Check for degenerate triangle
            if (areaSquared <= 0.0001f)
            {
