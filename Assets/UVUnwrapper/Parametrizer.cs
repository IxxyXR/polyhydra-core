...
namespace Unwrapper
{
    ...
    {
        public static bool Parametrize(List<Vector3> vertices, List<List<int>> faces, List<Vector2> vertexUvs)
        {
            ...
            try
            {
                ...
                {
                    vertexUvs.Add(new Vector2(uv.x, uv.y));
                }
        ...
        private static List<int> FindBoundaryLoop(List<List<int>> faces)
        {
        ...
        private static void ImproveMapping(List<Vector2> uvs, List<List<int>> faces, int iterations)
        {
        ...
        private static HashSet<int> GetVertexNeighbors(int vertexIndex, List<List<int>> faces)
        {
            ...
            foreach (var face in faces)
            {
                for (int i = 0; i < 3; i++)
                {
                    ...
                    {
                        neighbors.Add(face[(i + 1) % 3]);
                        neighbors.Add(face[(i + 2) % 3]);
                    }
                }
            }
            return neighbors;
